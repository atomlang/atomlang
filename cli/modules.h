...
// A function callback called by atomlang VM to get attribute of a native
// instance.
void objGetAttrib(PKVM* vm, void* instance, uint32_t id, PkStringPtr attrib);

// A function callback called by atomlang VM to set attribute of a native
// instance.
bool objSetAttrib(PKVM* vm, void* instance, uint32_t id, PkStringPtr attrib);

// The free callback of the object, that'll called by atomlang when a
// atomlang native instance garbage collected.
void freeObj(PKVM* vm, void* instance, uint32_t id);

// The native instance get_name callback used to get the name of a native
// instance from atomlang. Here the id we're using is the ObjType enum.
