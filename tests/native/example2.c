...
// bind it with with the atomlang VM.
...
// An enum value of native object, used as unique of the type in atomlang.
typedef enum {
  OBJ_VECTOR = 0,
} ObjType;
...
// Get name callback, will called from atomlang to get the type name from
...
void objGetAttrib(PKVM* vm, void* instance, uint32_t id, PkStringPtr attrib) {
  ...
  // Since we haven't used pkReturn...() function, atomlang VM already
