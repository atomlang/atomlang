...
SYSTEM_TO_BINARY_PATH = {
  "Windows": "..\\build\\debug\\bin\\atomlang.exe",
  "Linux": "../build/debug/atomlang",
  "Darwin": "../build/debug/atomlang",
}
...
def run_all_tests():
  ## get the interpreter.
  atomlang = get_atomlang_binary()
  
  for suite in TEST_SUITE:
    print_title(suite)
    for test in TEST_SUITE[suite]:
      path = join(THIS_PATH, test)
      run_test_file(atomlang, test, path)

def run_test_file(atomlang, test, path):
  FMT_PATH = "%-25s"
  INDENTATION = '  | '
  print(FMT_PATH % test, end='')

  sys.stdout.flush()
  result = run_command([atomlang, path])
...
## This will return the path of the atomlang binary (on different platforms).
## The debug version of it for enabling the assertions.
def get_atomlang_binary():
  system = platform.system()
  if system not in SYSTEM_TO_BINARY_PATH:
    error_exit("Unsupported platform %s" % system)

  atomlang = abspath(join(THIS_PATH, SYSTEM_TO_BINARY_PATH[system]))
  if not os.path.exists(atomlang):
    error_exit("atomlang interpreter not found at: '%s'" % atomlang)
