...
// Check if the 2 atomlang strings are equal.
#define IS_STR_EQ(s1, s2)          \
 (((s1)->hash == (s2)->hash) &&    \
 ((s1)->length == (s2)->length) && \
 (memcmp((const void*)(s1)->data, (const void*)(s2)->data, (s1)->length) == 0))

// Compare atomlang string with c string.
...
// Type definition of atomlang heap allocated types.
...
// Type enums of the atomlang heap allocated types.
typedef enum {
  OBJ_STRING,
  OBJ_LIST,
  OBJ_MAP,
