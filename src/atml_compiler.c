...
struct Compiler {
  ...
  const ATMLCompileOptions* options; //< To configure the compilation.
...
static void reportError(Compiler* compiler, const char* file, int line,
  ...
  vm->config.error_fn(vm, ATML_ERROR_COMPILE, file, line, message);
}
...
static void eatString(Compiler* compiler, bool single_quote) {
  ...
  while (true) {
    ...
    if (c == '\\') {
      switch (eatChar(compiler)) {
        case '"':  ATMLByteBufferWrite(&buff, compiler->vm, '"'); break;
        case '\'': ATMLByteBufferWrite(&buff, compiler->vm, '\''); break;
        case '\\': ATMLByteBufferWrite(&buff, compiler->vm, '\\'); break;
        case 'n':  ATMLByteBufferWrite(&buff, compiler->vm, '\n'); break;
        case 'r':  ATMLByteBufferWrite(&buff, compiler->vm, '\r'); break;
        case 't':  ATMLByteBufferWrite(&buff, compiler->vm, '\t'); break;
      ...
      ATMLByteBufferWrite(&buff, compiler->vm, c);
    }
  }
  ...
  ATMLByteBufferClear(&buff, compiler->vm);
...
static void compilerInit(Compiler* compiler, ATMLVM* vm, const char* source,
                         Script* script, const ATMLCompileOptions* options) {
...
static int compilerAddConstant(Compiler* compiler, Var value) {
  ATMLVarBuffer* literals = &compiler->script->literals;
  ...
  if (literals->count < MAX_CONSTANTS) {
    ATMLVarBufferWrite(literals, compiler->vm, value);
  } else {
...
static int emitByte(Compiler* compiler, int byte) {
  ...
  ATMLByteBufferWrite(&_FN->opcodes, compiler->vm,
                    (uint8_t)byte);
  ATMLUintBufferWrite(&_FN->oplines, compiler->vm,
...
static int compileType(Compiler* compiler) {
  ...
  while (next != TK_END && next != TK_EOF) {
    ...
    ATMLUintBufferWrite(&type->field_names, compiler->vm, f_index);
...
static Script* importFile(Compiler* compiler, const char* path) {
  ...
  // Resolve the path.
  ATMLStringPtr resolved = { path, NULL, NULL };
  ...
  ATMLStringPtr source = vm->config.load_script_fn(vm, path_name->data);
...
ATMLResult compile(ATMLVM* vm, Script* script, const char* source,
  ...
  // the native api function (ATMLNewModule() that'll return a module without a
  ...
  ATMLByteBufferClear(&script->body->fn->opcodes, vm);
  ...
  if (compiler->has_errors) {
    if (compiler->options && compiler->options->repl_mode &&
      ...
      return ATML_RESULT_UNEXPECTED_EOF;
    }
    return ATML_RESULT_COMPILE_ERROR;
  }
  return ATML_RESULT_SUCCESS;
}

ATMLResult ATMLCompileModule(ATMLVM* vm, ATMLHandle* module, ATMLStringPtr source,
                         const ATMLCompileOptions* options) {
  ...
  ATMLResult result = compile(vm, script, source.string, options);
