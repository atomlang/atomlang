...
// pkNewModule implementation (see atomlang.h for description).
PkHandle* pkNewModule(PKVM* vm, const char* name) {
  Script* module = newModuleInternal(vm, name);
  return vmNewHandle(vm, VAR_OBJ(module));
}

// pkModuleAddGlobal implementation (see atomlang.h for description).
...
// pkModuleAddFunction implementation (see atomlang.h for description).
...
// pkGetArgc implementation (see atomlang.h for description).
int pkGetArgc(const PKVM* vm) {
  __ASSERT(vm->fiber != NULL, "This function can only be called at runtime.");
  return ARGC;
}

// pkCheckArgcRange implementation (see atomlang.h for description).
...
// pkGetArg implementation (see atomlang.h for description).
...
// pkGetArgBool implementation (see atomlang.h for description).
...
// pkGetArgNumber implementation (see atomlang.h for description).
...
// pkGetArgString implementation (see atomlang.h for description).
...
// pkGetArgInstance implementation (see atomlang.h for description).
...
// pkGetArgValue implementation (see atomlang.h for description).
...
// pkReturnNull implementation (see atomlang.h for description).
void pkReturnNull(PKVM* vm) {
  RET(VAR_NULL);
}

// pkReturnBool implementation (see atomlang.h for description).
void pkReturnBool(PKVM* vm, bool value) {
  RET(VAR_BOOL(value));
}

// pkReturnNumber implementation (see atomlang.h for description).
void pkReturnNumber(PKVM* vm, double value) {
  RET(VAR_NUM(value));
}

// pkReturnString implementation (see atomlang.h for description).
void pkReturnString(PKVM* vm, const char* value) {
  RET(VAR_OBJ(newString(vm, value)));
}

// pkReturnStringLength implementation (see atomlang.h for description).
void pkReturnStringLength(PKVM* vm, const char* value, size_t length) {
  RET(VAR_OBJ(newStringLength(vm, value, (uint32_t)length)));
}

// pkReturnValue implementation (see atomlang.h for description).
void pkReturnValue(PKVM* vm, PkVar value) {
  RET(*(Var*)value);
}

// pkReturnHandle implementation (see atomlang.h for description).
void pkReturnHandle(PKVM* vm, PkHandle* handle) {
  RET(handle->value);
}

// pkReturnInstNative implementation (see atomlang.h for description).
...
// Index is could be larger than 32 bit integer, but the size in atomlang
...
#define MODULE_ADD_FN(module, name, fn, argc) \
  ...
  // constant and atomlang doesn't support constant) so the user shouldn't
  ...
  switch (obj->type) {
    ...
    case OBJ_RANGE:
    {
      ...
      switch (attrib->hash) {
        ...
        // We can't use 'start', 'end' since 'end' in atomlang is a
