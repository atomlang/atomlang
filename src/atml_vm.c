...
static ATMLResult runFiber(ATMLVM* vm, Fiber* fiber);

ATMLConfiguration ATMLNewConfiguration(void) {
...
ATMLCompileOptions ATMLNewCompilerOptions(void) {
  ATMLCompileOptions options;
  options.debug = false;
  // TODO:
...
ATMLVM* ATMLNewVM(ATMLConfiguration* config) {

  ATMLConfiguration default_config = ATMLNewConfiguration();

  if (config == NULL) config = &default_config;

  ATMLVM* vm = (ATMLVM*)config->realloc_fn(NULL, sizeof(ATMLVM), config->user_data);
  memset(vm, 0, sizeof(ATMLVM));
...
void* ATMLGetUserData(const ATMLVM* vm) {
  return vm->config.user_data;
}

void ATMLSetUserData(ATMLVM* vm, void* user_data) {
  vm->config.user_data = user_data;
}

ATMLHandle* ATMLNewHandle(ATMLVM* vm, ATMLVar value) {
  return vmNewHandle(vm, *((Var*)value));
}

ATMLVar ATMLGetHandleValue(const ATMLHandle* handle) {
  return (ATMLVar)&handle->value;
}

void ATMLReleaseHandle(ATMLVM* vm, ATMLHandle* handle) {
...
ATMLResult ATMLInterpretSource(ATMLVM* vm, ATMLStringPtr source, ATMLStringPtr path,
  ...
  // Compile the source.
  ATMLResult result = compile(vm, scr, source.string, options);
  if (source.on_done) source.on_done(vm, source);
  if (result != ATML_RESULT_SUCCESS) return result;
...
ATMLResult ATMLRunFiber(ATMLVM* vm, ATMLHandle* fiber,
  ...
  if (!vmPrepareFiber(vm, _fiber, argc, args)) {
    return ATML_RESULT_RUNTIME_ERROR;
  }
...
ATMLResult ATMLResumeFiber(ATMLVM* vm, ATMLHandle* fiber, ATMLVar value) {
  __ASSERT(fiber != NULL, "Handle fiber was NULL.");
  Var fb = fiber->value;
  __ASSERT(IS_OBJ_TYPE(fb, OBJ_FIBER), "Given handle is not a fiber.");
  Fiber* _fiber = (Fiber*)AS_OBJ(fb);
...
void ATMLSetRuntimeError(ATMLVM* vm, const char* message) {
...
ATMLHandle* vmNewHandle(ATMLVM* vm, Var value) {
  ATMLHandle* handle = (ATMLHandle*)ALLOCATE(vm, ATMLHandle);
  handle->value = value;
  handle->prev = NULL;
...
void vmCollectGarbage(ATMLVM* vm) {
  ...
  // Mark the handles.
  for (ATMLHandle* h = vm->handles; h != NULL; h = h->next) {
...
static inline bool resolveScriptPath(ATMLVM* vm, ATMLStringPtr* path_string) {
  if (vm->config.resolve_path_fn == NULL) return true;

  const char* path = path_string->string;
  ATMLStringPtr resolved;
...
// script) set to false.
static inline Var importScript(ATMLVM* vm, String* path_name) {
...
static inline void growStack(ATMLVM* vm, int size) {
  Fiber* fiber = vm->fiber;
...
static inline void pushCallFrame(ATMLVM* vm, const Function* fn, Var* rbp) {
...
static inline void reuseCallFrame(ATMLVM* vm, const Function* fn) {
...
static void reportError(ATMLVM* vm) {
  ...
  Fiber* fiber = vm->fiber;
  vm->config.error_fn(vm, ATML_ERROR_RUNTIME, NULL, -1, fiber->error->data);
  for (int i = fiber->frame_count - 1; i >= 0; i--) {
    ...
    vm->config.error_fn(vm, ATML_ERROR_STACKTRACE, fn->owner->path->data, line,
                        fn->name);
  }
}
...
#define DISPATCH()   goto L_vm_main_loop
  ...
  SWITCH() {
    ...
    OPCODE(LIST_APPEND):
    {
      ...
      ATMLVarBufferWrite(&((List*)AS_OBJ(list))->elements, vm, elem);
      DROP(); // elem
      DISPATCH();
    }
    ...
    OPCODE(INST_APPEND):
    {
      ...
      Inst* ins = inst_p->ins;
      ATMLVarBufferWrite(&ins->fields, vm, value);
      DROP(); // value
    ...
    OPCODE(TAIL_CALL):
    {
      ...
      *(call_fiber->ret) = VAR_NULL; //< Set the return value to null.
      ...
      if (fn->is_native) {
        ...
        if (vm->fiber == NULL) return ATML_RESULT_SUCCESS;
  ...
  UNREACHABLE(); //return ATML_RESULT_SUCCESS;
}