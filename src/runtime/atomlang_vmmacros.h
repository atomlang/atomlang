#ifndef __ATOMLANG_VMMACROS__
#define __ATOMLANG_VMMACROS__
...
#define ATOMLANG_VM_DEBUG                0               
#define ATOMLANG_VM_STATS                0               
#define ATOMLANG_GC_STATS                0               
#define ATOMLANG_GC_STRESSTEST           0               
#define ATOMLANG_GC_DEBUG                0               
#define ATOMLANG_STACK_DEBUG             0               
#define ATOMLANG_TRUST_USERCODE          0               
                                                        
                                                        

#if ATOMLANG_STACK_DEBUG
#define DEBUG_STACK()                   atomlang_stack_dump(fiber)
#else
#define DEBUG_STACK()
#endif

#if ATOMLANG_GC_DEBUG
#define DEBUG_GC(...)                   printf(__VA_ARGS__);printf("\n");fflush(stdout)
#else
#define DEBUG_GC(...)
#endif

#if ATOMLANG_VM_DEBUG
...
#if ATOMLANG_TRUST_USERCODE
#define BEGIN_TRUST_USERCODE(_vm)
#define END_TRUST_USERCODE(_vm)
#else
#define BEGIN_TRUST_USERCODE(_vm)       atomlang_gc_setenabled(_vm, false)
#define END_TRUST_USERCODE(_vm)         atomlang_gc_setenabled(_vm, true)
#endif

#if ATOMLANG_VM_STATS
#define RESET_STATS(_vm)                bzero(_vm->nstat, sizeof(_vm->nstat)); bzero(_vm->tstat, sizeof(_vm->tstat))
#define PRINT_STATS(_vm)                atomlang_vm_stats(_vm)
...
#define RUNTIME_ERROR(...)              do {                                                                \
                                            STORE_FRAME();                                                  \
                                            report_runtime_error(vm, ATOMLANG_ERROR_RUNTIME, __VA_ARGS__);   \
                                            return false;                                                   \
                                        } while (0)
...
#define RUNTIME_WARNING(...)            do {                                                            \
                                            report_runtime_error(vm, ATOMLANG_WARNING, __VA_ARGS__);     \
                                        } while (0)
...
#define PUSH_FRAME(_c,_s,_r,_n)     atomlang_callframe_t *cframe = atomlang_new_callframe(vm, fiber);                 \
                                    ...
                                    cframe->args = (USE_ARGS(_c)) ? atomlang_list_from_array(vm, _n-1, _s+1) : NULL
...
#define DEFINE_STACK_VARIABLE(_v,_r)                register atomlang_value_t _v = STACK_GET(_r)
#define DEFINE_INDEX_VARIABLE(_v,_r)                register atomlang_value_t _v = (_r < MAX_REGISTERS) ? STACK_GET(_r) : VALUE_FROM_INT(_r-MAX_REGISTERS)
...
#define PREPARE_FUNC_CALLN(_c,_i,_w,_N)             atomlang_closure_t *_c = (atomlang_closure_t *)atomlang_class_lookup_closure(atomlang_value_getclass(v2), cache[_i]); \
                                                    ...
                                                    if (!atomlang_check_stack(vm, fiber, stacktopdelta, &stackstart)) return false;              \
...
#define CALL_FUNC(_name,_c,r1,nargs,rwin)           atomlang_fiber_t *current_fiber = fiber;                             \
                                                    STORE_FRAME();                                                      \
                                                    execute_op_##_name:                                                 \
                                                    switch(_c->f->tag) {                                                \
...
#define SETMETA_INITED(c)                           atomlang_class_get_meta(c)->is_inited = true
#define GET_VALUE(_idx)                             args[_idx]
#define RETURN_VALUE(_v,_i)                         do {atomlang_vm_setslot(vm, _v, _i); return true;} while(0)
#define RETURN_CLOSURE(_v,_i)                       do {atomlang_vm_setslot(vm, _v, _i); return false;} while(0)
...
#define RETURN_ERROR(...)                           do {                                                                                    \
                                                        ...
                                                        atomlang_fiber_seterror(atomlang_vm_fiber(vm), (const char *) _buffer);               \
...
#define DECLARE_1VARIABLE(_v,_idx)                  register atomlang_value_t _v = GET_VALUE(_idx)
...
#define NEW_FUNCTION(_fptr)                         (atomlang_function_new_internal(NULL, NULL, _fptr, 0))
#define NEW_CLOSURE_VALUE(_fptr)                    ((atomlang_value_t){.isa = atomlang_class_closure,.p = (atomlang_object_t *)atomlang_closure_new(NULL, NEW_FUNCTION(_fptr))})

#define FUNCTION_ISA_SPECIAL(_f)                    (OBJECT_ISA_FUNCTION(_f) && (_f->tag == EXEC_TYPE_SPECIAL))
#define FUNCTION_ISA_DEFAULT_GETTER(_f)             ((_f->index < ATOMLANG_COMPUTED_INDEX) && (_f->special[EXEC_TYPE_SPECIAL_GETTER] == NULL))
#define FUNCTION_ISA_DEFAULT_SETTER(_f)             ((_f->index < ATOMLANG_COMPUTED_INDEX) && (_f->special[EXEC_TYPE_SPECIAL_SETTER] == NULL))
...
#define FUNCTION_ISA_BRIDGED(_f)                    (_f->index == ATOMLANG_BRIDGE_INDEX)
