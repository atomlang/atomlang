...
ATMLVarType ATMLGetValueType(const ATMLVar value) {
  __ASSERT(value != NULL, "Given value was NULL.");

  if (IS_NULL(*(const Var*)(value))) return ATML_NULL;
  if (IS_BOOL(*(const Var*)(value))) return ATML_BOOL;
  if (IS_NUM(*(const Var*)(value))) return ATML_NUMBER;
  ...
  switch (obj->type) {
    case OBJ_STRING: return ATML_STRING;
    case OBJ_LIST:   return ATML_LIST;
    case OBJ_MAP:    return ATML_MAP;
    case OBJ_RANGE:  return ATML_RANGE;
    case OBJ_SCRIPT: return ATML_SCRIPT;
    case OBJ_FUNC:   return ATML_FUNCTION;
    case OBJ_FIBER:  return ATML_FIBER;
    case OBJ_CLASS:  return ATML_CLASS;
    case OBJ_INST:   return ATML_INST;
  }

  UNREACHABLE();
}

ATMLHandle* ATMLNewString(ATMLVM* vm, const char* value) {
  String* str = newString(vm, value);
  vmPushTempRef(vm, &str->_super); 
  ATMLHandle* handle = vmNewHandle(vm, VAR_OBJ(str));
  vmPopTempRef(vm); 
  return handle;
}

ATMLHandle* ATMLNewStringLength(ATMLVM* vm, const char* value, size_t len) {
...
ATMLHandle* ATMLNewList(ATMLVM* vm) {
  List* list = newList(vm, MIN_CAPACITY);
  vmPushTempRef(vm, &list->_super);
  ATMLHandle* handle = vmNewHandle(vm, VAR_OBJ(list));
  vmPopTempRef(vm);
  return handle;
}

ATMLHandle* ATMLNewMap(ATMLVM* vm) {
  Map* map = newMap(vm);
  vmPushTempRef(vm, &map->_super); // map
  ATMLHandle* handle = vmNewHandle(vm, VAR_OBJ(map));
  vmPopTempRef(vm); // map
  return handle;
}

ATMLHandle* ATMLNewFiber(ATMLVM* vm, ATMLHandle* fn) {
  ...
  ATMLHandle* handle = vmNewHandle(vm, VAR_OBJ(fiber));
  vmPopTempRef(vm); // fiber
  return handle;
}

ATMLHandle* ATMLNewInstNative(ATMLVM* vm, void* data, uint32_t id) {
  Instance* inst = newInstanceNative(vm, data, id);
  vmPushTempRef(vm, &inst->_super); // inst
  ATMLHandle* handle = vmNewHandle(vm, VAR_OBJ(inst));
...
void ATMLByteBufferAddString(ATMLByteBuffer* self, ATMLVM* vm, const char* str,
                           uint32_t length) {
  ATMLByteBufferReserve(self, vm, self->count + length);
...
#define MARK_OBJ_BUFFER(m_name)                                   \
  void mark##m_name##Buffer(ATMLVM* vm, ATML##m_name##Buffer* self) { \
    if (self == NULL) return;                                     \
    for (uint32_t i = 0; i < self->count; i++) {                  \
...
static void popMarkedObjectsInternal(Object* obj, ATMLVM* vm) {
...
static String* _allocateString(ATMLVM* vm, size_t length) {
...
List* newList(ATMLVM* vm, uint32_t size) {
  ...
  ATMLVarBufferInit(&list->elements);
  if (size > 0) {
    ATMLVarBufferFill(&list->elements, vm, VAR_NULL, size);
...
Script* newScript(ATMLVM* vm, String* name, bool is_core) {
  ...
  ATMLVarBufferInit(&script->globals);
  ATMLUintBufferInit(&script->global_names);
  ATMLVarBufferInit(&script->literals);
  ATMLFunctionBufferInit(&script->functions);
  ATMLClassBufferInit(&script->classes);
  ATMLStringBufferInit(&script->names);
...
Function* newFunction(ATMLVM* vm, const char* name, int length, Script* owner,
  ...
  if (owner == NULL) {
    ...
    ATMLFunctionBufferWrite(&owner->functions, vm, func);
  ...
  if (is_native) {
    ...
    ATMLByteBufferInit(&fn->opcodes);
    ATMLUintBufferInit(&fn->oplines);
    fn->stack_size = 0;
    func->fn = fn;
  }
...
Class* newClass(ATMLVM* vm, Script* scr, const char* name, uint32_t length) {
  ...
  ATMLClassBufferWrite(&scr->classes, vm, type);
  type->owner = scr;
  type->name = scriptAddName(scr, vm, name, length);
  ATMLUintBufferInit(&type->field_names);
...
Instance* newInstance(ATMLVM* vm, Class* ty, bool initialize) {
  ...
  inst->ins = ins;
  ins->type = ty;
  ATMLVarBufferInit(&ins->fields);

  if (initialize && ty->field_names.count != 0) {
    ATMLVarBufferFill(&ins->fields, vm, VAR_NULL, ty->field_names.count);
  }
...
List* rangeAsList(ATMLVM* vm, Range* self) {
  ...
  if (self->from < self->to) {
    ...
    for (double i = self->from; i < self->to; i++) {
      ATMLVarBufferWrite(&list->elements, vm, VAR_NUM(i));
    }
    return list;
...
void listInsert(ATMLVM* vm, List* self, uint32_t index, Var value) {
  ...
  ATMLVarBufferWrite(&self->elements, vm, VAR_NULL);
...
List* listJoin(ATMLVM* vm, List* l1, List* l2) {
  ...
  vmPushTempRef(vm, &list->_super);
  ATMLVarBufferConcat(&list->elements, vm, &l1->elements);
  ATMLVarBufferConcat(&list->elements, vm, &l2->elements);
  vmPopTempRef(vm);
...
static void _mapResize(ATMLVM* vm, Map* self, uint32_t capacity) {
...
void freeObject(ATMLVM* vm, Object* self) {
  switch (self->type) {
    ...
    case OBJ_LIST:
      ATMLVarBufferClear(&(((List*)self)->elements), vm);
      break;
    ...
    case OBJ_SCRIPT: {
      ...
      ATMLVarBufferClear(&scr->globals, vm);
      ATMLUintBufferClear(&scr->global_names, vm);
      ATMLVarBufferClear(&scr->literals, vm);
      ATMLFunctionBufferClear(&scr->functions, vm);
      ATMLClassBufferClear(&scr->classes, vm);
      ATMLStringBufferClear(&scr->names, vm);
    } break;

    case OBJ_FUNC: {
      Function* func = (Function*)self;
      if (!func->is_native) {
        ATMLByteBufferClear(&func->fn->opcodes, vm);
        ATMLUintBufferClear(&func->fn->oplines, vm);
    ...
    case OBJ_CLASS: {
      ...
      ATMLUintBufferClear(&type->field_names, vm);
    } break;
    ...
    case OBJ_INST:
    {
      ...
      if (inst->is_native) {
        ...
        Inst* ins = inst->ins;
        ATMLVarBufferClear(&ins->fields, vm);
        DEALLOCATE(vm, ins);
      }
...
uint32_t scriptAddName(Script* self, ATMLVM* vm, const char* name,
  ...
  ATMLStringBufferWrite(&self->names, vm, new_name);
  vmPopTempRef(vm);
...
uint32_t scriptAddGlobal(ATMLVM* vm, Script* script,
  ...
  ATMLUintBufferWrite(&script->global_names, vm, name_ind);
  ATMLVarBufferWrite(&script->globals, vm, value);
...
void scriptAddMain(ATMLVM* vm, Script* script) {
  ...
  const char* fn_name = ATML_IMPLICIT_MAIN_NAME;
...
bool instGetAttrib(ATMLVM* vm, Instance* inst, String* attrib, Var* value) {
  ...
  if (inst->is_native) {
    ...
    if (vm->config.inst_get_attrib_fn) {
      ...
      // below var 'val' so that the users can use 'ATMLReturn...()' function
      // to return the attribute as well.
      Var* temp = vm->fiber->ret;
      Var val = VAR_UNDEFINED;

      vm->fiber->ret = &val;
      ATMLStringPtr attr = { attrib->data, NULL, NULL,
...
bool instSetAttrib(ATMLVM* vm, Instance* inst, String* attrib, Var value) {
  ...
  if (inst->is_native) {
    ...
    if (vm->config.inst_set_attrib_fn) {
      // Temproarly change the fiber's "return address" to points to the
      // below var 'ret' so that the users can use 'ATMLGetArg...()' function
...
const char* getATMLVarTypeName(ATMLVarType type) {
  switch (type) {
    case ATML_NULL:     return "Null";
    case ATML_BOOL:     return "Bool";
    case ATML_NUMBER:   return "Number";
    case ATML_STRING:   return "String";
    case ATML_LIST:     return "List";
    case ATML_MAP:      return "Map";
    case ATML_RANGE:    return "Range";
    case ATML_SCRIPT:   return "Script";
    case ATML_FUNCTION: return "Function";
    case ATML_FIBER:    return "Fiber";
    case ATML_CLASS:    return "Class";
    case ATML_INST:     return "Inst";
  }
...
static void _toStringInternal(ATMLVM* vm, const Var v, ATMLByteBuffer* buff,
                              OuterSequence* outer, bool repr) {
  ASSERT(outer == NULL || repr, OOPS);

  if (IS_NULL(v)) {
    ATMLByteBufferAddString(buff, vm, "null", 4);
    return;

  } else if (IS_BOOL(v)) {
    if (AS_BOOL(v)) ATMLByteBufferAddString(buff, vm, "true", 4);
    else ATMLByteBufferAddString(buff, vm, "false", 5);
    return;
    ...
    if (isnan(value)) {
      ATMLByteBufferAddString(buff, vm, "nan", 3);

    } else if (isinf(value)) {
      if (value > 0.0) {
        ATMLByteBufferAddString(buff, vm, "+inf", 4);
      } else {
        ATMLByteBufferAddString(buff, vm, "-inf", 4);
      }
      ...
      ATMLByteBufferAddString(buff, vm, num_buff, length);
    }
    ...
    switch (obj->type) {
      ...
      case OBJ_STRING:
      {
        ...
        if (outer == NULL && !repr) {
          ATMLByteBufferAddString(buff, vm, str->data, str->length);
          return;
        } else {
          // If recursive return with quotes (ex: [42, "hello", 0..10]).
          ATMLByteBufferWrite(buff, vm, '"');
          for (const char* c = str->data; *c != '\0'; c++) {
            switch (*c) {
              case '"': ATMLByteBufferAddString(buff, vm, "\\\"", 2); break;
              case '\\': ATMLByteBufferAddString(buff, vm, "\\\\", 2); break;
              case '\n': ATMLByteBufferAddString(buff, vm, "\\n", 2); break;
              case '\r': ATMLByteBufferAddString(buff, vm, "\\r", 2); break;
              case '\t': ATMLByteBufferAddString(buff, vm, "\\t", 2); break;

              default:
                ATMLByteBufferWrite(buff, vm, *c);
                break;
            }
          }
          ATMLByteBufferWrite(buff, vm, '"');
      ...
      case OBJ_LIST:
      {
        ...
        if (list->elements.count == 0) {
          ATMLByteBufferAddString(buff, vm, "[]", 2);
          return;
        }
        ...
        while (seq != NULL) {
          if (seq->is_list && seq->list == list) {
            ATMLByteBufferAddString(buff, vm, "[...]", 5);
            return;
          }
          seq = seq->outer;
        }
        ...
        for (uint32_t i = 0; i < list->elements.count; i++) {
          if (i != 0) ATMLByteBufferAddString(buff, vm, ", ", 2);
      ...
      case OBJ_MAP:
      {
        ...
        do {
          ...
          if (!_first) ATMLByteBufferAddString(buff, vm, ", ", 2);
      ...
      case OBJ_RANGE:
      {
        ...
        ATMLByteBufferAddString(buff, vm, "[Range:", 7);
        ATMLByteBufferAddString(buff, vm, buff_from, len_from);
        ATMLByteBufferAddString(buff, vm, "..", 2);
        ATMLByteBufferAddString(buff, vm, buff_to, len_to);
        ATMLByteBufferWrite(buff, vm, ']');
        return;
      }
      ...
      case OBJ_SCRIPT: {
        ...
        ATMLByteBufferAddString(buff, vm, "[Module:", 8);
        if (scr->module != NULL) {
          ATMLByteBufferAddString(buff, vm, scr->module->data,
                              scr->module->length);
        } else {
          ATMLByteBufferWrite(buff, vm, '"');
          ATMLByteBufferAddString(buff, vm, scr->path->data, scr->path->length);
          ATMLByteBufferWrite(buff, vm, '"');
        }
        ATMLByteBufferWrite(buff, vm, ']');
        return;
      ...
      case OBJ_FUNC: {
        ...
        ATMLByteBufferAddString(buff, vm, "[Func:", 6);
        ATMLByteBufferAddString(buff, vm, fn->name, (uint32_t)strlen(fn->name));
        ATMLByteBufferWrite(buff, vm, ']');
        return;
      }
      ...
      case OBJ_FIBER: {
        ...
        ATMLByteBufferAddString(buff, vm, "[Fiber:", 7);
        ATMLByteBufferAddString(buff, vm, fb->func->name,
      ...
      case OBJ_CLASS: {
        ...
        ATMLByteBufferAddString(buff, vm, "[Class:", 7);
      ...
      case OBJ_INST:
      {
        ...
        ATMLByteBufferWrite(buff, vm, '[');
        ATMLByteBufferAddString(buff, vm, inst->name,
        ...
        if (!inst->is_native) {
          ...
          for (uint32_t i = 0; i < ty->field_names.count; i++) {
            if (i != 0) ATMLByteBufferWrite(buff, vm, ',');

            ATMLByteBufferWrite(buff, vm, ' ');
            String* f_name = ty->owner->names.data[ty->field_names.data[i]];
            ATMLByteBufferAddString(buff, vm, f_name->data, f_name->length);
            ATMLByteBufferWrite(buff, vm, '=');
          ...
          ATMLByteBufferAddString(buff, vm, buff_addr, (uint32_t)len);
        }
