...
// atomlanglang visibility macros. define PK_DLL for using atomlang as a 
// shared library and define PK_COMPILE to export symbols when compiling the
// atomlang it self as a shared library.
...
// atomlangLang Virtual Machine. It'll contain the state of the execution, stack,
// heap, and manage memory allocations.
typedef struct PKVM PKVM;

// A handle to the atomlang variables. It'll hold the reference to the
...
// A temproary pointer to the atomlang variable. This pointer is acquired
// from the atomlang's current stack frame and the pointer will become
// dangling once after the stack frame is popped.
typedef void* PkVar;

// Type enum of the atomlang variables, this can be used to get the type
...
// Type of the error message that atomlang will provide with the pkErrorFn
// callback.
typedef enum {
...
// Result that atomlang will return after a compilation or running a script
...
/*****************************************************************************/
/* ATOMLANG FUNCTION POINTERS & CALLBACKS                                  */
/*****************************************************************************/

// C function pointer which is callable from atomlangLang by native module
// functions.
...
// freed by by the garbage collector, to indicate that atomlang is done with
// the native instance.
typedef void (*pkInstFreeFn) (PKVM* vm, void* instance, uint32_t id);

// A function callback to get the name of the native instance from atomlang,
// using it's [id]. The returned string won't be copied by atomlang so it's
// expected to be alived since the instance is alive and recomended to return
// a C literal string.
typedef const char* (*pkInstNameFn) (uint32_t id);

// A get arribute callback, called by atomlang VM when trying to get an attribute
...
// if the '.as_string' attribute doesn't exists, atomlang VM will use a default
// to string value.
...
// atomlang VM will handle it, On success update the native instance and return
...
/*****************************************************************************/
/* ATOMLANG PUBLIC TYPE DEFINES                                            */
/*****************************************************************************/

// A string pointer wrapper to pass c string between host application and
// atomlang VM. With a on_done() callback to clean it when the atomlang VM is done
// with the string.
struct PkStringPtr {
  ...
  // These values are provided by the atomlang VM to the host application, you're
  // not expected to set this when provideing string to the atomlang VM.
...
// that min <= max, and atomlang won't validate this in release binary.
