...
void *atomlang_realloc(atomlang_vm *vm, void *ptr, size_t new_size) {
    if (vm && (new_size >= atomlang_vm_maxmemblock(vm))) {
        atomlang_vm_seterror(vm, "Maximum memory re-allocation block size reached (req: %d, max: %lld).", new_size, (int64_t)atomlang_vm_maxmemblock(vm));
        return NULL;
    }
