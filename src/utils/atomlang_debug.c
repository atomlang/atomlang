...
const char *atomlang_disassemble (atomlang_vm *vm, atomlang_function_t *f, const char *bcode, uint32_t blen, bool deserialize) {
    uint32_t    *ip = NULL;
    ...
    if (deserialize) {
        ...
        ip = atomlang_bytecode_deserialize(bcode, blen, &ninsts);
        ...
        switch (op) {
            ...
            case LOADK: {
                ...
                if (index < CPOOL_INDEX_MAX) {
                    ...
                    if (f) {
                        char b[2018];
                        atomlang_value_t constant = atomlang_function_cpool_get(f, index);
                        atomlang_value_dump(vm, constant, b, sizeof(b));
                        --bindex; 
